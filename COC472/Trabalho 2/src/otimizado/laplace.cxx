...
inline double seconds(void) {
    static const double secs_per_tick = 1.0 / CLOCKS_PER_SEC;  return ( (double) clock() ) * secs_per_tick;
}
...
Real LaplaceSolver :: timeStep(const Real dt) {
    ...
    for (int i=1; i<nx-1; ++i) { 
        for (int j=1; j<ny-1; ++j) { 
            tmp = u[i][j]; 
            u[i][j] = ((u[i-1][j] + u[i+1][j])*dy2 +  (u[i][j-1] + u[i][j+1])*dx2)*factor;
            tmpErr = u[i][j] - tmp;
            err += tmpErr*tmpErr;  
        } 
    } 
...
int main(int argc, char * argv[]) {
    ...
    t_start = seconds(); 
    std::cout << s.solve(n_iter, eps) << std::endl;  t_end = seconds(); 
